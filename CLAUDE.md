...
무인 자동화 SNS 봇). **계산기 5개(3.1~3.5) 전부 구현·배포 완료(2026-08-17 부산 세션), 계산
로직도 직접 재검증 완료.** 남은 건 **PM이 확정한 서비스 명칭 변경**("부동산 계산기" 대문,
"by 리얼티북" 부제, 탭 라벨 일부 변경 — README "6. 다음 단계 백로그" 최상단 참고), 4장 디자인
리스타일, 앱인토스 SDK 연동, SNS 자동화 봇. 명칭 변경은 어차피 4장 리스타일과 같은 헤더/탭
컴포넌트를 건드리므로 한 번에 처리 권장.
...
## 참고 — 브라우저 자동화 테스트 이슈 (2026-08-17)

계산 로직(Node 직접 실행)은 전부 검증 완료했지만, 프로덕션 페이지에서 브라우저 자동화로 탭
클릭 등 인터랙션 테스트가 안 됐음(도구가 반응 없음, 콘솔 에러는 없음 — README "9." 참고).
앱 자체 버그인지 도구 문제인지 미확정 — 다음 세션에서 재확인하거나 형이 직접 한 번 눌러서
탭 전환/영수증카드 저장이 정상 동작하는지 확인 필요.
